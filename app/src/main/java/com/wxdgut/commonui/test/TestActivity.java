            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        // 在这里执行您想要延时执行的代码
                        runOnUiThread(() -> {
                            CommonDialog.newBuilder(TestActivity.this).layout(R.layout.dialog_chat).widthMatch(true).showAsDropDown(btn2, Gravity.LEFT, 0, 0).dimAmount(0.0f).anim(CommonDialog.DEFAULT_ANIM).build();
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        // 在这里执行您想要延时执行的代码
                        runOnUiThread(() -> {
                            CommonDialog.newBuilder(TestActivity.this).build();
