...
public class BaseAdapter<T> extends RecyclerView.Adapter<CommonViewHolder> {
    ...
    public interface MyItemClickListener {
        void onItemClick(View view, int actualPosition, boolean isChange);
    }

    //长点击事件接口
    public interface MyItemLongClickListener {
        void onItemLongClick(View view, int actualPosition, boolean isChange);
    }
    ...
    public interface OnBindDataListener<T> {
        ...
        void onBindViewHolder(T model, CommonViewHolder viewHolder, int type, int originalPosition);
    }

    //绑定多类型的数据接口（item多布局样式，如标题栏、内容item）
    //根据position确定当前item，拿到item后需要返回该item对应的布局类型，类型由用户定义
    ...
    private void handleClick(MyItemClickListener listener, CommonViewHolder holder) {
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                state[0] = !state[0];
                myItemClickListener.onItemClick(holder.itemView, actualPosition, state[0]);
            }
        });
    }
    ...
    private void handleLongClick(MyItemLongClickListener listener, CommonViewHolder holder) {
        ...
        holder.itemView.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                int actualPosition = holder.getLayoutPosition();
                state[0] = !state[0];
                myItemLongClickListener.onItemLongClick(holder.itemView, actualPosition, state[0]);
